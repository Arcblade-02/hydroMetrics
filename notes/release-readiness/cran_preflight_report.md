...
- `tests and coverage`: `FAIL` - Test commands status: FAIL / PASS; coverage overall: 86.59%.
- `vignette build and documentation regeneration`: `FAIL` - Vignette command status: PASS; documentation command status: PASS; README present: FALSE; NEWS present: FALSE.
...
- Package source version on this branch is `0.1.0`, which should be compared against the requested `0.2.x` target.
