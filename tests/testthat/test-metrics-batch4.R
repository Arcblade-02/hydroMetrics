test_that("batch4 metrics return expected hm_result structure and ids", {
  out <- evaluate_metrics(
    sim = c(1, 2, 3),
    obs = c(1, 2, 1),
    metrics = c("me", "d", "md", "obs_normalized_agreement_index", "slope_scaled_r2")
  )

  expect_s3_class(out, "hm_result")
  expect_true(is.data.frame(out))
  expect_identical(colnames(out), c("metric", "name", "value"))
  expect_identical(out$metric, c("me", "d", "md", "obs_normalized_agreement_index", "slope_scaled_r2"))
})

test_that("batch4 perfect-fit values are correct", {
  sim <- c(1, 2, 3)
  obs <- c(1, 2, 3)
  out <- evaluate_metrics(sim, obs, c("me", "d", "md", "obs_normalized_agreement_index", "slope_scaled_r2"))
  values <- setNames(out$value, out$metric)

  expect_equal(values[["me"]], 0)
  expect_equal(values[["d"]], 1)
  expect_equal(values[["md"]], 1)
  expect_equal(values[["obs_normalized_agreement_index"]], 1)
  expect_equal(values[["slope_scaled_r2"]], 1)
})
...
test_that("batch4 deterministic numeric formulas match inline computations", {
  ...
  expect_equal(values[["obs_normalized_agreement_index"]], 1 - sum(rd_rel_err^2) / rd_denom)
  expect_equal(values[["slope_scaled_r2"]], br2_expected)
})

test_that("obs_normalized_agreement_index errors when obs contains zero", {
  expect_error(
    evaluate_metrics(c(1, 2, 3), c(1, 0, 2), "obs_normalized_agreement_index"),
    "obs contains zero"
  )
})
...
test_that("slope_scaled_r2 errors when sd(obs) is zero", {
  expect_error(
    evaluate_metrics(c(1, 2, 3), c(2, 2, 2), "slope_scaled_r2"),
    "sd\\(obs\\) == 0"
  )
})
