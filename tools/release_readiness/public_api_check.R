run_public_api_check <- function(context) {
  ...
  code <- paste(
    ...
    "sim <- c(1, 2, 3, 4, 5)",
    "obs <- c(1, 2, 2.5, 4.5, 5)",
    "runtime_targets <- c('gof', 'ggof', 'preproc', 'valindex', 'metric_search', 'metric_preset')",
    ...
    "      tryCatch({",
    "        value <- switch(name,",
    "          gof = do.call(fn, list(sim = sim, obs = obs)),",
    "          ggof = do.call(fn, list(sim = sim, obs = obs)),",
    "          preproc = do.call(fn, list(sim = sim, obs = obs)),",
    "          valindex = do.call(fn, list(sim = sim, obs = obs, fun = c('nse', 'rmse'))),",
    "          metric_search = do.call(fn, list(preset = 'recommended')),",
    "          metric_preset = do.call(fn, list(preset = 'recommended')),",
    "          stop('No runtime probe configured for this target.')",
    "        )",
    ...
    ")",
    "for (target in c('preproc', 'valindex', 'ggof', 'metric_search', 'metric_preset')) {",
    "  if (!(target %in% exports)) {",
    "    runtime_results <- c(runtime_results, sprintf('- `%s` => unavailable: not exported from clean session.', target))",
    "    next",
    "  }",
    "  fn <- get(target, envir = ns, inherits = FALSE)",
    "  value <- tryCatch(switch(target,",
    "    preproc = do.call(fn, list(sim = sim, obs = obs)),",
    "    valindex = do.call(fn, list(sim = sim, obs = obs, fun = c('nse', 'rmse'))),",
    "    ggof = do.call(fn, list(sim = sim, obs = obs)),",
    "    metric_search = do.call(fn, list(preset = 'recommended')),",
    ...
    "  runtime_results <- c(runtime_results, sprintf('- `%s` => %s', target, rendered))",
    "}",
  ...
  rr_result(
    ...
    summary = if (length(missing_required) == 0L && identical(run$status, 0L)) {
      "All required current public entry points are exported and callable from a clean session."
    } else {
      sprintf(
        "Required current public surface is incomplete on the current snapshot; missing or non-exported: %s.",
